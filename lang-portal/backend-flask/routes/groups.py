...
def load(app):
  ...
  @app.route('/<int:group_id>/words/raw', methods=['GET'])
  def get_group_words_raw(group_id):
    ...
    # Query to get all words in the group with their stats
    query = """
        SELECT 
            w.id,
            w.japanese,
    ...
    words = cursor.execute(query, (group_id, group_id, group_id)).fetchall()
